...
use crate::crd::{Schema, ValueOrRefSource, LocalObjectReference};
...
pub fn resolve_namespace(reference: &LocalObjectReference, resource_ns: &str) -> String {
    reference.namespace.clone().unwrap_or_else(|| resource_ns.to_string())
}
    ...
    if let Some(ref from) = source.value_from {
        if let Some(ref secret_ref) = from.secret_key_ref {
            ...
            let secret = secrets_api.get(&secret_ref.name).await
                .map_err(|e| Error::ConfigError(format!("Failed to read secret {}: {}", secret_ref.name, e)))?;
            
            if let Some(ref data) = secret.data {
                if let Some(bytes) = data.get(&secret_ref.key) {
                    let val = String::from_utf8(bytes.0.clone())
                        .map_err(|e| Error::ConfigError(format!("Secret key {} is not valid UTF-8: {}", secret_ref.key, e)))?;
                    return Ok(val);
                }
            }
            return Err(Error::ConfigError(format!("Key {} not found in secret {}", secret_ref.key, secret_ref.name)));
        }
        ...
        if let Some(ref cm_ref) = from.config_map_ref {
            ...
            let cm = cm_api.get(&cm_ref.name).await
                .map_err(|e| Error::ConfigError(format!("Failed to read ConfigMap {}: {}", cm_ref.name, e)))?;
            ...
            return Err(Error::ConfigError(format!("Key {} not found in ConfigMap {}", cm_ref.key, cm_ref.name)));
        }
    }

    Err(Error::ConfigError("ValueOrRefSource has neither value nor valueFrom specified".to_string()))
}
